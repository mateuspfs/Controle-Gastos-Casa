...
const navItems: NavItem[] = [
  { label: 'Pessoas', icon: UserGroupIcon, path: '/pessoas' },
  { label: 'Transações', icon: ArrowsRightLeftIcon, path: '/transacoes' },
  { label: 'Categorias', icon: TagIcon, path: '/categorias' },
];
      ...
      <aside className="hidden h-screen w-72 flex-col border-r border-slate-200/80 bg-white/95 backdrop-blur-sm px-4 py-6 shadow-lg dark:border-slate-800 dark:bg-slate-900 dark:shadow-none md:flex">
        <div className="flex items-center justify-between px-2">
          <div>
            ...
            <h2 className="text-lg font-semibold">Gastos Casa</h2>
          </div>
          <ThemeToggle compact />
        </div>
      ...
      <div className="md:hidden">
        <div className="flex items-center justify-between border-b border-slate-200 bg-white px-4 py-3 dark:border-slate-800 dark:bg-slate-900">
          <button
            ...
            className="rounded-lg border border-slate-200 bg-white p-2.5 text-slate-700 shadow-sm transition hover:bg-slate-100 dark:border-slate-800 dark:bg-slate-800 dark:text-slate-100"
